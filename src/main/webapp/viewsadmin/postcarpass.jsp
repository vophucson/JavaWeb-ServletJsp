		...
		<div class="hk-pg-header">
			<h4 class="hk-pg-title">
				<span class="pg-title-icon"><span class="feather-icon"><i
						data-feather="archive"></i></span></span>Danh sách bài đăng đã duyệt
			</h4>
		</div>
		<!-- /Title -->
										...
										<tbody>
											<c:forEach items="${list}" var="l">
												<tr>
													...
													<td>Đã Duyệt <a
														href="admin-changepass?pid=${l.productId}" class="mr-25"
														data-toggle="tooltip" data-original-title="Duyệt bài đăng">
															<i class="icon-pencil"></i>
													</a>
													</td>
													<td><a href="admin-deletepostpass?pid=${l.productId}"
											...
											<ul class="pagination custom-pagination pagination-filled">
												<c:if test="${tag > 1}">
													<li class="page-item"><a class="page-link"
														href="admin-postcarpass?index=${tag - 1}"><i
															class="ion ion-ios-arrow-round-back"></i></a></li>
												</c:if>
												<c:forEach end="${endP}" begin="1" var="i">
													<li class="page-item ${tag==i?"active":""}"><a
														class="page-link" href="admin-postcarpass?index=${i}">${i}</a></li>
												</c:forEach>
												<c:if test="${tag < endP}">
													<li class="page-item"><a class="page-link"
														href="admin-postcarpass?index=${tag + 1}"><i
